...
static auto dirWinDlg = L"D:\\Stuff\\Core\\cpp\\windlg\\windlg";
static auto dirTargets = L"D:\\Stuff\\Core\\cpp";
static auto targets = {
...
void processSolution(std::wstring_view target)
{
	auto solutionPath = lib::str::fmt(L"%s\\%s\\%s.vcxproj", dirTargets, target, target);
	auto lines = lib::str::splitLines(lib::FileMapped::ReadAllStr(solutionPath));

	auto refill = [](const std::vector<std::wstring>& lines,
			std::wstring_view tagSuffix,
			std::wstring_view ext) -> std::vector<std::wstring> {
		...
		for (const auto& srcFile : lib::path::dirList( lib::str::fmt(L"%s\\*.%s", dirWinDlg, ext) )) {
	...
	lines = refill(lines, L"Compile", L"cpp");
	lines = refill(lines, L"Include", L"h");
	lib::File::EraseAndWriteStr(solutionPath, lib::str::join(lines, L"\r\n"));
}

void processFilter(std::wstring_view target)
{
	auto filterPath = lib::str::fmt(L"%s\\%s\\%s.vcxproj.filters", dirTargets, target, target);
...
int main()
{
	for (auto&& target : targets) {
		...
		processSolution(target);
		processFilter(target);
	}
